...
Policy MUST implement the `IModule` and the `IPolicy` interface and have module type id : `5`

```solidity
interface IPolicy is IModule {
    /**
     * @dev Validates a UserOperation
     * @param id the id for differentiating the context
     * @param userOp the ERC-4337 PackedUserOperation
     *
     * MUST validate that the signature is a valid signature of the userOpHash
     */
     ...
     * MUST NOT modify state
     */
    function checkSignaturePolicy(bytes32 id, address sender, bytes32 hash, bytes calldata signature)
        external
        view
        returns (uint256);

    /**
     * @dev Returns if the module was already initialized for a provided smartaccount and id
     * @param account address of the account to check
     * @param id context id
     */
    function isInitialized(address account, bytes32 id) external view returns(bool);
}
```

#### Signer 

Signer MUST implement the `IModule` and the `ISigner` interface and have module type id : `6`

```solidity
interface ISigner is IModule {
    /**
     * @dev Validates a UserOperation and userOpHash
     * @param id the id for differentiating the context
     * @param userOp the ERC-4337 PackedUserOperation
     * @param userOpHash the hash of the ERC-4337 PackedUserOperation
     *
...
As an extension, policy and signer has been introduced to enable the modular permission system. Although policy and signer can function as validation, it has additional feature.

1. `bytes32 id` to cover cases where a module can be called on different context
2. separation between userOpHash validation(Signer) and userOp validation(Policy)

`id` can be defined on account side to differentiate the context.

Module(Policy and Signer) MUST treat first 32 bytes on `onInstall()` and `onUninstall()` as `id`.
